...
struct ScreeningTestView: View {
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack {
                                ForEach(viewModel.fetchedImages, id: \.url) { item in
                                    KFImageView(url: item.url, height: 100)
                                        .padding(.trailing, 4)
                                }
                            }
                            .padding(.horizontal)
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack {
                                ForEach(viewModel.safeImagesForDisplay, id: \.url) { item in
                                    KFImageView(url: item.url, height: 150)
                                        .overlay(
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack {
                                ForEach(viewModel.unsafeImagesForDisplay, id: \.url) { result in
                                    VStack {
                                        KFImageView(url: result.url, height: 150)
                                            .overlay(
                                                RoundedRectangle(cornerRadius: 8)
    ...
    @ViewBuilder
    private func KFImageView(url: URL, height: CGFloat) -> some View {
        KFImage(url)
            .placeholder {
                Rectangle()
