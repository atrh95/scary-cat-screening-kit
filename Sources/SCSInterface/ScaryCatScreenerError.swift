        ...
        switch self {
            case .resourceBundleNotFound:
                ...
                userInfo[NSLocalizedFailureReasonErrorKey] =
                    "Essential application resources might be missing or improperly packaged."
            case let .modelLoadingFailed(originalError):
                code = 2
                userInfo[NSLocalizedDescriptionKey] = "The ML model failed to load."
                if let originalError {
                    userInfo[NSLocalizedFailureReasonErrorKey] =
                        "The model file might be corrupt, inaccessible, or incompatible."
                    userInfo[NSUnderlyingErrorKey] = originalError as NSError
                } else {
                    userInfo[NSLocalizedFailureReasonErrorKey] =
                        "The model file might be missing or inaccessible, with no further details."
                }
            case .modelNotFound:
                code = 3
                userInfo[NSLocalizedDescriptionKey] = "The ML model file was not found."
                userInfo[NSLocalizedFailureReasonErrorKey] =
                    "The specified model file does not exist at the expected location."
            case let .predictionFailed(originalError):
                code = 4
                userInfo[NSLocalizedDescriptionKey] = "An error occurred during the image screening prediction process."
                if let originalError {
                    userInfo[NSLocalizedFailureReasonErrorKey] =
                        "There was an issue with the Vision framework request or processing its results."
                    userInfo[NSUnderlyingErrorKey] = originalError as NSError
                } else {
                    userInfo[NSLocalizedFailureReasonErrorKey] =
                        "An unspecified issue occurred with the Vision framework request or result processing."
                }
        }
